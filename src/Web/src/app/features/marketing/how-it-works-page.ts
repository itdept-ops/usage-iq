...
export class HowItWorksPage {
  ...
  readonly faqs: Faq[] = [
    ...
    {
      q: 'Where does it run?',
      a: 'Wherever you want. It is a .NET 9 API, an Angular front end, and PostgreSQL — packaged with Docker Compose. The live demo runs on a single small AWS instance behind auto-HTTPS.',
    },
    {
      q: 'How is cost calculated?',
      a: 'From a per-model pricing table you control, with input, output, and all three cache tiers priced independently. Change a rate and recompute history in one click.',
    },
    {
      q: 'Who can sign in?',
      a: 'Only Google accounts you have approved. Permissions are enforced per-request on the server, and every request is written to an audit log.',
    },
  ];
}
